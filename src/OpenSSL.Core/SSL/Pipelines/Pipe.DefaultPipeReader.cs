...
namespace OpenSSL.Core.SSL.Pipelines
{
    ...
    public abstract partial class Pipe
    {
        private sealed class DefaultPipeReader : PipeReader, IValueTaskSource<ReadResult>
        {
            ...
            public void OnCompleted(Action<object> continuation, object state, short token, ValueTaskSourceOnCompletedFlags flags) => _pipe.OnReadAsyncCompletedInternal(continuation, state, flags);
        }
    }
}
